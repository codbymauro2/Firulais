...
import {
  fetchAdoptions, createAdoption, updateAdoption, deleteAdoption,
  uploadAdminImage, type Adoption, type AdoptionInput,
} from "../../lib/adminService";
import ImageCropModal from "../../components/ImageCropModal";

const EMPTY: AdoptionInput = {
  name: "", age: "", gender: "Macho", description: "",
  shelter: "", location: "", image_url: null, contact_url: "", is_active: true,
};
...
export default function AdminAdoptionsScreen() {
  ...
  const [editing, setEditing] = useState<Adoption | null>(null);
  const [form, setForm] = useState<AdoptionInput>(EMPTY);
  ...
  const load = () => {
    setLoading(true);
    fetchAdoptions().then(setItems).finally(() => setLoading(false));
  };

  useEffect(load, []);

  const openCreate = () => { setEditing(null); setForm(EMPTY); setImageFile(null); setModalOpen(true); };
  const openEdit = (item: Adoption) => {
    setEditing(item);
    setForm({ name: item.name, age: item.age ?? "", gender: item.gender ?? "Macho", description: item.description ?? "", shelter: item.shelter ?? "", location: item.location ?? "", image_url: item.image_url, contact_url: item.contact_url ?? "", is_active: item.is_active });
    setImageFile(null);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (imageFile) {
        image_url = await uploadAdminImage(imageFile, "pet-images", "adoptions");
      }
      const payload = { ...form, image_url };
      if (editing) await updateAdoption(editing.id, payload);
      else await createAdoption(payload);
      setModalOpen(false);
      load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { await deleteAdoption(deleteId); load(); }
    catch (e) { alert(e instanceof Error ? e.message : "Error al eliminar"); }
    finally { setDeleteId(null); }
  };

  const set = (k: keyof AdoptionInput, v: string | boolean | null) =>
    setForm((f) => ({ ...f, [k]: v }));
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-left px-4 py-3 font-semibold text-slate-500 hidden md:table-cell">Refugio</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-500 hidden sm:table-cell">Género</th>
            ...
            <tbody>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 overflow-hidden shrink-0">
                        {item.image_url
                          ...
                          : <div className="w-full h-full flex items-center justify-center"><span className="material-symbols-outlined text-slate-300 text-[20px]">pets</span></div>
                        }
                      </div>
                      <div>
                        <p className="font-semibold">{item.name}</p>
                        <p className="text-slate-400 text-xs">{item.age}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-600 hidden md:table-cell">{item.shelter ?? "—"}</td>
                  <td className="px-4 py-3 text-slate-600 hidden sm:table-cell">{item.gender ?? "—"}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={5} className="text-center py-12 text-slate-400">No hay adopciones aún</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            <h3 className="text-lg font-bold mb-5">{editing ? "Editar adopción" : "Nueva adopción"}</h3>
            <div className="flex flex-col gap-4">
              <Field label="Nombre *" value={form.name} onChange={(v) => set("name", v)} />
              <div className="grid grid-cols-2 gap-3">
                <Field label="Edad" value={form.age ?? ""} onChange={(v) => set("age", v)} />
                <div>
              ...
              <Field label="Refugio" value={form.shelter ?? ""} onChange={(v) => set("shelter", v)} />
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saving || !form.name} className="flex-1 h-11 rounded-xl bg-[#2b9dee] text-white text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-2">
                {saving ? <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg> : null}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm text-center">
            <p className="font-bold mb-2">¿Eliminar esta adopción?</p>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors ${value ? "bg-[#2b9dee]" : "bg-slate-300"}`}
      >
        ...
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-xl bg-slate-100 overflow-hidden shrink-0">
            {preview
              ...
              : <div className="w-full h-full flex items-center justify-center"><span className="material-symbols-outlined text-slate-300 text-[22px]">image</span></div>
            }
          </div>
