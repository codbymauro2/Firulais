...
import {
  fetchHappyEndings, createHappyEnding, updateHappyEnding, deleteHappyEnding,
  uploadAdminImage, type HappyEnding, type HappyEndingInput,
} from "../../lib/adminService";
import ImageCropModal from "../../components/ImageCropModal";

const EMPTY: HappyEndingInput = {
  pet_name: "", breed: "", days_lost: null, story: "", image_url: null, is_active: true,
};

export default function AdminHappyEndingsScreen() {
  const [items, setItems] = useState<HappyEnding[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<HappyEnding | null>(null);
  const [form, setForm] = useState<HappyEndingInput>(EMPTY);
  ...
  const load = () => {
    setLoading(true);
    fetchHappyEndings().then(setItems).finally(() => setLoading(false));
  };

  useEffect(load, []);

  const openCreate = () => { setEditing(null); setForm(EMPTY); setImageFile(null); setModalOpen(true); };
  const openEdit = (item: HappyEnding) => {
    setEditing(item);
    setForm({ pet_name: item.pet_name, breed: item.breed ?? "", days_lost: item.days_lost, story: item.story ?? "", image_url: item.image_url, is_active: item.is_active });
    setImageFile(null);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (imageFile) image_url = await uploadAdminImage(imageFile, "pet-images", "happy-endings");
      const payload = { ...form, image_url };
      if (editing) await updateHappyEnding(editing.id, payload);
      else await createHappyEnding(payload);
      setModalOpen(false);
      load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { await deleteHappyEnding(deleteId); load(); }
    catch (e) { alert(e instanceof Error ? e.message : "Error al eliminar"); }
    finally { setDeleteId(null); }
  };

  const set = (k: keyof HappyEndingInput, v: string | boolean | number | null) =>
    setForm((f) => ({ ...f, [k]: v }));
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                <th className="text-left px-4 py-3 font-semibold text-slate-500">Mascota</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-500 hidden md:table-cell">Raza</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-500 hidden sm:table-cell">Días desaparecido</th>
            ...
            <tbody>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 overflow-hidden shrink-0">
                        {item.image_url
                          ? <img src={item.image_url} alt={item.pet_name} className="w-full h-full object-cover" />
                          : <div className="w-full h-full flex items-center justify-center"><span className="material-symbols-outlined text-slate-300 text-[20px]">favorite</span></div>
                        }
                      </div>
                      <p className="font-semibold">{item.pet_name}</p>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-600 hidden md:table-cell">{item.breed ?? "—"}</td>
                  <td className="px-4 py-3 text-slate-600 hidden sm:table-cell">{item.days_lost != null ? `${item.days_lost} días` : "—"}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={5} className="text-center py-12 text-slate-400">No hay historias aún</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            <h3 className="text-lg font-bold mb-5">{editing ? "Editar historia" : "Nueva historia"}</h3>
            <div className="flex flex-col gap-4">
              <Field label="Nombre de la mascota *" value={form.pet_name} onChange={(v) => set("pet_name", v)} />
              <div className="grid grid-cols-2 gap-3">
                <Field label="Raza" value={form.breed ?? ""} onChange={(v) => set("breed", v)} />
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-1 block">Días desaparecido</label>
                  <input
                    type="number" min="0"
                    value={form.days_lost ?? ""}
                    onChange={(e) => set("days_lost", e.target.value ? parseInt(e.target.value) : null)}
                    className="w-full h-10 rounded-xl border border-slate-200 px-3 text-sm focus:outline-none focus:border-[#2b9dee]"
                  />
                </div>
              </div>
              <Textarea label="Historia" value={form.story ?? ""} onChange={(v) => set("story", v)} />
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saving || !form.pet_name} className="flex-1 h-11 rounded-xl bg-[#2b9dee] text-white text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm text-center">
            <p className="font-bold mb-2">¿Eliminar esta historia?</p>
